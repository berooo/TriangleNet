...
def test_triangle_net(sources, prefix='TCGA', target='logs/log_TriangleNet', csv_target='logs/predict_trianglenet.csv'):
    val = [(fn, source) for source in sources for fn in os.listdir(source) if fn[:len(prefix)] == prefix]
    disc = 20
    solver = TTAFrame(TriangleNet)
    solver.load('weights/TriangleNet.th')
    ...
    for i, t in enumerate(val):
        ...
        contour_path = os.path.join(gt_root, name[:23] + '_contour' + name[23:])
        ...
        ground_truth = np.concatenate([content, contour], axis = 1)
...
if __name__ == '__main__':
    # test_triangle_net([os.path.join(Constants.ROOT, 'test', 'images')])
    ...
    for slide in slides:
        test_triangle_net(sources, slide, f'logs/log_{slide}', f'logs/predict_{slide}.csv')