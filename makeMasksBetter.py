...
class MaskMaker:
    ...
    def __init__(self, width, height):
        self.width = width
        self.height = height
        self.image = np.zeros((width // 2, height // 2, 3), np.uint8)

    def line(self, p1, p2, color=(255, 255, 255), width=10):
        cv.line(self.image, (p1[0] // 2, p1[1] // 2), (p2[0] // 2, p2[1] // 2), (color[2], color[1], color[0]), width, cv.LINE_AA)
    ...
    for dn in os.listdir(dataDir):
        if dn.find('.') == -1:
            ...
            if svs != '' and xml != '':
                ...
                mm = MaskMaker(slide.dimensions[1], slide.dimensions[0])
                ...
                if os.path.exists(f'{filename}_content.png'):
                    os.remove(f'{filename}_content.png')
